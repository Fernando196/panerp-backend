...
import { CreateCharolaDto } from '../../domain/dto/create-charola.dto';
...
export class CharolasService {
  ...
  async findAll(estado?: string) {
    ...
    return this.charolaRepo.find({ where, order: { numero_interno: 'ASC' } });
  }
  ...
  async create(dto: CreateCharolaDto) {
    const existe = await this.charolaRepo.findOne({ where: { numero_interno: dto.numero_interno } });
