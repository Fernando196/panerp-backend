...
export class NotificacionesService {
  ...
  async findByUsuario(usuarioId: string, soloNoLeidas = false) {
    ...
    return this.notifRepo.find({ where, order: { created_at: 'DESC' }, take: 50 });
  }
  ...
  async marcarTodasLeidas(usuarioId: string) {
    await this.notifRepo.update({ usuario_id: usuarioId, leida: false }, { leida: true });
  ...
  private async crear(data: { tipo: string; titulo: string; mensaje: string; usuario_id?: string }) {
    const notif = this.notifRepo.create({
      id: uuid(),
      usuario_id: data.usuario_id ?? 'broadcast',
